...
Describe 'Write-TuiBox' {
    ...
    Context 'Frame count without border' {
        It 'returns body-line count for body only' {
            $count = Write-TuiBox -Body @('one') 6>$null
            $count | Should -Be 1
        }
        It 'returns header + body + footer with no separators' {
            $count = Write-TuiBox -Header @('h') -Body @('b') -Footer @('f') 6>$null
            $count | Should -Be 3
        }
        It 'handles multi-line sections' {
            $count = Write-TuiBox -Header @('h1','h2') -Body @('b1','b2','b3') -Footer @('f1') 6>$null
            $count | Should -Be 6
        }
    }
    ...
    Context 'Truncation does not change frame count' {
        It 'still returns body-line count when a body line is truncated' {
            $longLine = 'x' * 200
            $count = Write-TuiBox -Body @($longLine) -Border -MaxWidth 20 6>$null
        ...
        It 'preserves frame count for an ANSI-styled long line' {
            ...
            $count = Write-TuiBox -Body @($ansiLong) -Border -MaxWidth 20 6>$null
            $count | Should -Be 3
        }
    }
}
