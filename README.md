...
|--------|-------|-------------------|-------|
| `Length` | meter, centimeter, millimeter, micrometer, decameter, kilometer, inch, foot, yard, mile, parsec | `m`, `cm`, `mm`, `µm`, `um`, `dam`, `km`, `in`, `inches`, `"`, `″`, `ft`, `feet`, `'`, `′`, `yd`, `mi`, `pc` | Base = meter. Per-region default: Imperial → `foot`, Metric → `meter`. `parsec` uses the IAU 2015 exact definition (648000/π au ≈ 3.086×10¹⁶ m) — astronomical scale only; pass `-OutputUnit parsec` with custom `-Min`/`-Max` (or `-DefaultsByUnit`) since the meter UnitDefaults' Max=10000 wouldn't fit even a fraction of one. |
